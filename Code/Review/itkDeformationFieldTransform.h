...
protected:
  ...
  typename DeformationFieldType::PixelType ReorientVectorByDirection(typename DeformationFieldType::PointType point )
  {
    enum { ImageDimension = DeformationFieldType::ImageDimension };
    typename InterpolatorType::ContinuousIndexType cidx;
    this->m_DeformationField->TransformPhysicalPointToContinuousIndex( point, cidx );
    typename InterpolatorType::OutputType vec = this->m_Interpolator->EvaluateAtContinuousIndex( cidx );
    typename DeformationFieldType::PixelType newvec;
    newvec.Fill(0);
    for (unsigned int row=0; row<ImageDimension; row++)
      for (unsigned int col=0; col<ImageDimension; col++)
        newvec[row]+=vec[col]*this->m_DeformationField->GetDirection()[row][col];
    return newvec;
  }
