...
{
  return this->GetLocalDeformation( point );
}

template<class TScalar, unsigned int NDimensions>
typename DeformationFieldTransform<TScalar, NDimensions>::JacobianType &
DeformationFieldTransform<TScalar, NDimensions>
::GetLocalDeformation( const InputPointType & point ) const
{
  IndexType idx;
  this->m_DeformationField->TransformPhysicalPointToIndex( point, idx );
  return this->GetLocalDeformation( idx );
}
...
{
  JacobianType * jMatrix = new JacobianType;
  jMatrix->SetSize( NDimensions, NDimensions );
  jMatrix->Fill(0.0);
