...
NeighborhoodNormalizedCrossCorrelationImageToImageMetric<TFixedImage,
        TMovingImage>::NeighborhoodNormalizedCrossCorrelationImageToImageMetric() {
    //TODO: modify the callback function.


    this->m_ValueAndDerivativeThreader->SetThreadedGenerateData(
            Self::ComputeMetricValueInRegionOnTheFlyThreaded);
 ...
 */
template<class TFixedImage, class TMovingImage>
void NeighborhoodNormalizedCrossCorrelationImageToImageMetric<TFixedImage,
        TMovingImage>::PrintSelf(std::ostream & os, Indent indent) const {
    Superclass::PrintSelf(os, indent);
    os << indent << "Parameters: " << m_Parameters << std::endl;
}

template<class TFixedImage, class TMovingImage>
unsigned int NeighborhoodNormalizedCrossCorrelationImageToImageMetric<
        TFixedImage, TMovingImage>::GetNumberOfParameters() const {
    return 5;
}

//TODO: this is the thread callback
template<class TFixedImage, class TMovingImage>
static void NeighborhoodNormalizedCrossCorrelationImageToImageMetric<
        TFixedImage, TMovingImage>::ComputeMetricValueInRegionOnTheFlyThreaded(
        const ImageRegionType &regionForThread, int threadId, Self *holder) {
    ...
    // typename MetricType::MeasureType local_cc;
    ...
    holder->InitializeScanning(regionForThread, scan_it, scan_mem, scan_para);
    // holder->measure_per_thread[threadId] = NumericTraits<InternalComputationValueType>::Zero;

    scan_it.GoToBegin();
    for (; !scan_it.IsAtEnd(); ++scan_it) {
        holder->UpdateQueues(scan_it, scan_mem, scan_para);
        holder->ComputeInformationFromQueues(scan_it, scan_mem, scan_para);
        holder->ComputeUpdate(scan_it, scan_mem, scan_para, deriv, local_cc);
        // holder->ComputeUpdateBothDirection(scan_it, scan_mem, scan_para, deriv, derivInv, local_cc);


        //TODO: modify this from Michael, put localDerivative back to the transform derivative
        nU.Value() += deriv;
        nUinv.Value() += derivInv;
        ;

        // TODO: modify this according to Michael's interface
...
template<class TFixedImage, class TMovingImage>
void NeighborhoodNormalizedCrossCorrelationImageToImageMetric<TFixedImage,
        TMovingImage>::GetValueAndDerivative(MeasureType & value,
...
template<class TFixedImage, class TMovingImage>
typename NeighborhoodNormalizedCrossCorrelationImageToImageMetric<TFixedImage,
        TMovingImage>::MeasureType NeighborhoodNormalizedCrossCorrelationImageToImageMetric<
...
template<class TFixedImage, class TMovingImage>
void NeighborhoodNormalizedCrossCorrelationImageToImageMetric<TFixedImage,
        TMovingImage>::GetDerivative(const ParametersType &,
        DerivativeType & derivative) const {
    derivative.Fill(0.0);
}
...
template<class TFixedImage, class TMovingImage>
void NeighborhoodNormalizedCrossCorrelationImageToImageMetric<TFixedImage,
        TMovingImage>::Initialize(void) throw (itk::ExceptionObject) {
}

template<class TFixedImage, class TMovingImage>
void NeighborhoodNormalizedCrossCorrelationImageToImageMetric<TFixedImage,
        TMovingImage>::InitializeGradientCalculator() {
...
template<class TFixedImage, class TMovingImage>
void NeighborhoodNormalizedCrossCorrelationImageToImageMetric<TFixedImage,
        TMovingImage>::InitializeScanning(const ImageRegionType &scan_region,
...
template<class TFixedImage, class TMovingImage>
void NeighborhoodNormalizedCrossCorrelationImageToImageMetric<TFixedImage,
        TMovingImage>::UpdateQueuesAtBeginingOfLine(
    ...
    for (unsigned int i = nb_fill_zero; i < (2 * scan_para.r[0] + 1); i++) {
        ...
        for (unsigned int indct = i; indct < hoodlen; indct += (2
                * scan_para.r[0] + 1)) {
...
template<class TFixedImage, class TMovingImage>
void NeighborhoodNormalizedCrossCorrelationImageToImageMetric<TFixedImage,
        TMovingImage>::UpdateQueuesToNextScanWindow(
        const ScanningIteratorType &scan_it, ScanMemType &scan_mem,
        const ScanParaType &scan_para) {
    ...
    for (unsigned int indct = 2 * scan_para.r[0]; indct < hoodlen; indct += (2
            * scan_para.r[0] + 1)) {
        bool isInBounds = true;
        scan_it.GetPixel(indct, isInBounds);
        typename FixedImageType::IndexType index = scan_it.GetIndex(indct);
...
template<class TFixedImage, class TMovingImage>
void NeighborhoodNormalizedCrossCorrelationImageToImageMetric<TFixedImage,
        TMovingImage>::UpdateQueues(const ScanningIteratorType &scan_it,
...
template<class TFixedImage, class TMovingImage>
void NeighborhoodNormalizedCrossCorrelationImageToImageMetric<TFixedImage,
        TMovingImage>::ComputeInformationFromQueues(
        const ScanningIteratorType &scan_it, ScanMemType &scan_mem,
        const ScanParaType &scan_para) {
    ...
    if (count > 0) {
        ...
        float sff = suma2 - fixedMean * suma - fixedMean * suma + count
                * fixedMean * fixedMean;
        float smm = sumb2 - movingMean * sumb - movingMean * sumb + count
                * movingMean * movingMean;
        float sfm = sumab - movingMean * suma - fixedMean * sumb + count
                * movingMean * fixedMean;
...
template<class TFixedImage, class TMovingImage>
void NeighborhoodNormalizedCrossCorrelationImageToImageMetric<TFixedImage,
        TMovingImage>::ComputeUpdateBothDirection(
        const ScanningIteratorType &scan_it, ScanMemType &scan_mem,
        const ScanParaType &scan_para, VectorType &deriv, VectorType &derivInv,
        InternalComputationValueType &local_cc) {
